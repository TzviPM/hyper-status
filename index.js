// Require
const { shell } = require('electron');
const { exec } = require('child_process');
const tildify = require('tildify');

// Config
exports.decorateConfig = (config) => {
    const hyperStatus = Object.assign({
        footerTransparent: false,
        headerTransparent: false,
        dirtyColor: config.colors.lightYellow,
        arrowsColor: config.colors.blue,
        fontSize: config.fontSize,
        fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", "Roboto", "Oxygen", "Ubuntu", "Cantarell", "Fira Sans", "Droid Sans", "Helvetica Neue", sans-serif',
    }, config.hyperStatus);

    return Object.assign({}, config, {
        css: `
            ${config.css || ''}
            .terms_terms {
                margin-bottom: 30px;
            }
            .footer_footer {
                display: flex;
                justify-content: center;
                position: fixed;
                bottom: 0;
                left: 0;
                right: 0;
                z-index: 100;
                font-family: ${hyperStatus.fontFamily};
                font-size: ${hyperStatus.fontSize}px;
                height: 30px;
                padding: 0 14px 1px;
                opacity: ${hyperStatus.footerTransparent ? '0.5' : '1'};
                cursor: default;
                -webkit-user-select: none;
                transition: opacity 250ms ease;
            }
            .footer_footer:hover {
                opacity: 1;
            }
            .footer_footer:before {
                content: '';
                position: absolute;
                top: 0;
                left: 0;
                z-index: -1;
                width: 100%;
                height: 100%;
                border-bottom-left-radius: 4px;
                border-bottom-right-radius: 4px;
                background-color: ${config.foregroundColor || 'transparent'};
                opacity: 0.07;
            }
            .header_header.forTerm {
                display: flex;
                justify-content: space-between;
                z-index: 100;
                font-family: ${hyperStatus.fontFamily};
                font-size: ${hyperStatus.fontSize}px;
                height: 30px;
                padding: 0 14px 1px;
                opacity: ${hyperStatus.headerTransparent ? '0.5' : '1'};
                cursor: default;
                -webkit-user-select: none;
                transition: opacity 250ms ease;
            }
            .header_header.forTerm:hover {
                opacity: 1;
            }
            .item_item {
                position: relative;
                display: flex;
                align-items: center;
                color: ${config.foregroundColor || 'white'};
                white-space: nowrap;
                background-repeat: no-repeat;
                background-position: left center;
                opacity: 0;
                pointer-events: none;
            }
            .item_active, .item_item.item_time {
                opacity: 0.7;
                pointer-events: auto;
            }
            .item_active:before {
                content: '';
                position: absolute;
                top: 0;
                left: 0;
                width: 14px;
                height: 100%;
                -webkit-mask-repeat: no-repeat;
                -webkit-mask-position: left center;
                background-color: ${config.foregroundColor || 'white'};
            }
            .item_folder {
                display: inline-block;
                text-overflow: ellipsis;
                padding-left: 21px;
                overflow: hidden;
            }
            .item_folder:before {
                -webkit-mask-image: url('data:image/svg+xml;base64,');
                -webkit-mask-size: 14px 12px;
            }
            .item_branch {
                padding-left: 30px;
            }
            .item_branch:before {
                left: 14.5px;
                -webkit-mask-image: url('data:image/svg+xml;base64,');
                -webkit-mask-size: 9px 12px;
            }
            .item_click:hover {
                text-decoration: underline;
                cursor: pointer;
            }
            .item_folder, .item_text {
                line-height: 29px;
            }
            .item_text {
                height: 100%;
            }
            .item_icon {
                display: none;
                width: 12px;
                height: 100%;
                margin-left: 9px;
                -webkit-mask-size: 12px 12px;
                -webkit-mask-repeat: no-repeat;
                -webkit-mask-position: 0 center;
            }
            .icon_active {
                display: inline-block;
            }
            .icon_dirty {
                -webkit-mask-image: url('data:image/svg+xml;base64,');
                background-color: ${hyperStatus.dirtyColor};
            }
            .icon_push, .icon_pull {
                -webkit-mask-image: url('data:image/svg+xml;base64,');
                background-color: ${hyperStatus.arrowsColor};
            }
            .icon_pull {
                transform: scaleY(-1);
                -webkit-mask-position: 0 8px;
            }
        `
    })
};

// Current shell cwd
const getStuff = (pid, cb) => {
    exec(`lsof -p ${pid} | grep cwd | tr -s ' ' | cut -d ' ' -f9-`, (err, cwd) => {
        let curCwd = cwd.trim();
        setBranch(curCwd, cb);
    })
};

// Current git branch
const setBranch = (actionCwd, cb) => {
    exec(`git symbolic-ref --short HEAD`, { cwd: actionCwd }, (err, branch) => {
        let curBranch = branch;

        if (branch !== '') {
            setRemote(actionCwd, (remote) => {
              checkDirty(actionCwd, (dirty) => {
                checkArrows(actionCwd, (push, pull) => {
                  cb(actionCwd, curBranch, remote, dirty, push, pull);
                });
              });
            });
        }
        else {
          cb(actionCwd, curBranch);
        }
    })
};

// Current git remote
const setRemote = (actionCwd, cb) => {
    exec(`git config --get remote.origin.url`, { cwd: actionCwd }, (err, remote) => {
        let curRemote = remote.trim().replace(/^git@(.*?):/, 'https://$1/').replace(/[A-z0-9\-]+@/, '').replace(/\.git$/, '');
        cb(curRemote);
    })
};

// Check if repo is dirty
const checkDirty = (actionCwd, cb) => {
    exec(`git status --porcelain --ignore-submodules -unormal`, { cwd: actionCwd }, (err, dirty) => {
        cb(dirty);
    })
};

// Check git left & right arrows status
const checkArrows = (actionCwd, cb) => {
    exec(`git rev-list --left-right --count HEAD...@'{u}' 2>/dev/null`, { cwd: actionCwd }, (err, arrows) => {
        arrows = arrows.split('\t');
        let pushArrow = arrows[0] > 0 ? arrows[0] : '';
        let pullArrow = arrows[1] > 0 ? arrows[1] : '';
        cb(pushArrow, pullArrow);
    })
};

// Status line
exports.decorateHyper = (Hyper, { React }) => {
    return class extends React.Component {
        render() {
            return (
                React.createElement(Hyper, Object.assign({}, this.props, {
                    customChildren: React.createElement('footer', { className: 'footer_footer' },
                        React.createElement('div', { title: "Current time", className: 'item_item item_time' }, new Date().toLocaleTimeString())
                    )
                }))
            )
        }
        componentDidMount() {
            this.interval = setInterval(() => {
                this.forceUpdate();
            }, 1000)
        }
        componentWillUnmount() {
            clearInterval(this.interval)
        }
    };
};

// tab title bar
exports.decorateTerm = (Term, { React }) => {
    return class extends React.Component {
        constructor() {
            super();
            this.state = {};
        }
        render() {
            const hasFolder = this.state.folder ? ' item_active' : '';
            const hasBranch = this.state.branch ? ' item_active' : '';
            const isDirty = this.state.dirty ? ' icon_active' : '';
            const hasPush = this.state.push ? ' icon_active' : '';
            const hasPull = this.state.pull ? ' icon_active' : '';

            return (
                React.createElement(Term, Object.assign({}, this.props, {
                    customChildrenBefore: React.createElement('header', { className: 'header_header forTerm' },
                        React.createElement('div', { title: this.state.folder, className: `item_item item_folder${hasFolder}` }, this.state.folder ? tildify(String(this.state.folder)) : ''),
                        React.createElement('div', { title: this.state.remote, className: `item_item item_branch${hasBranch}` },
                            React.createElement('span', { className: 'item_text' }, this.state.branch),
                            React.createElement('i', { title: 'git-dirty', className: `item_icon icon_dirty${isDirty}` }),
                            React.createElement('i', { title: 'git-push', className: `item_icon icon_push${hasPush}` }),
                            React.createElement('i', { title: 'git-pull', className: `item_icon icon_pull${hasPull}` })
                        )
                    )
                }))
            )
        }
        componentDidMount() {
            this.interval = setInterval(() => {
              getStuff(this.props.pid, (folder, branch, remote, dirty, push, pull) => {
                this.setState({
                  folder,
                  branch,
                  remote,
                  dirty,
                  push,
                  pull
                })
              })
            }, 100)
        }
        componentWillUnmount() {
            clearInterval(this.interval)
        }
    };
};

exports.getTermProps = (uid, parentProps, props) => {
  return Object.assign({}, props, {
    pid: parentProps.sessions[uid].pid
  })
};
